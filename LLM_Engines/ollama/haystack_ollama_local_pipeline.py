...
def tennis_interactive_session(data_dir=None, persist_path=None, show_tokens=True, chunk_size=512, chunk_overlap=50):

    """
    Start an interactive Q&A session with the tennis data (XLSX files) in data/tennis.
    ...
    while True:
        try:
            question = input("Enter your tennis question: ").strip()
            if question.lower() in ['quit', 'exit', 'q']:
                session_duration = time.time() - session_start
                if show_tokens and question_count > 0:
                    print(f"\n{'='*60}")
            ...
            if answer is not None:
                question_count += 1
                if show_tokens:
                    question_tokens = estimate_tokens(question) + estimate_tokens(answer) + 500
                    total_session_tokens += question_tokens
                    print(f"Question {question_count} tokens: ~{question_tokens}")
                    print(f"Session total tokens: {total_session_tokens}\n")
        except KeyboardInterrupt:
...
if __name__ == "__main__":
    ...
    elif args.mode == "tennis_interactive":
        print("--- Starting Tennis Interactive Q&A Session ---")
        tennis_interactive_session()
